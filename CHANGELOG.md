...
- 主胶囊现在提供新建待办、记录灵感和同步到 Obsidian 的快捷入口；收起或展开状态会直接写明对应动作与数量。
- 空待办输入行显示「输入待办」提示；顶栏图标补充可访问名称，便于鼠标悬浮识别和辅助技术读取。

**Obsidian 同步与灵感标签**

- 笔记纸支持以标签整理灵感，并可在每日设定时间或通过设置、托盘立即同步当天待办和灵感到 Obsidian Vault 中的 Markdown 日记。

- 同步设置会显示上次同步状态；未配置 Vault 或路径无效时会给出明确提示，不会将文件写到 Vault 目录外。

- 同步只更新 Markdown 中由 PaperTodo 管理的内容块，保留用户在同一天日记里自行补充的内容。
